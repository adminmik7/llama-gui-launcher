...
- Добавлена кнопка «✕ Сбросить настройки» для сброса всех параметров к значениям по умолчанию
- Уменьшен размер шрифта лога (11 → 10)
- Исправлено пересечение элементов UI в блоке «Дополнительно»

### Ранее добавленные изменения
- Увеличен размер шрифта в панели логов (9 → 11)
- Убрано дублирующее сообщение "Сервер завершилось" из лога (остается только код выхода)
...
### Шаг 6: Управление настройками

- **💾 Сохранить конфиг** — сохраняет текущие параметры в JSON-файл
- **📂 Загрузить конфиг** — загружает параметры из JSON-файла
- **✕ Сбросить настройки** — сбрасывает все параметры к значениям по умолчанию с подтверждением
